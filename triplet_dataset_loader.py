...
class TripletGenerator:
    def __init__(self, datasetPath, split_ratio=(0.7, 0.2, 0.1), os_walk=False):
    ...
    def _get_label_names(self):
        ...
        if self.os_walk:
            ...
            for root, dirs, files in os.walk(self.datasetPath):
                # Only add directories that contain image files directly
                if any(file.lower().endswith(('.png', '.jpg', '.jpeg')) for file in files):
                    relative_path = os.path.relpath(root, self.datasetPath)
                    if relative_path != ".":
                        label_names.add(relative_path)
    ...
    def _split_label_images(self):
        ...
        for label, images in self.label_images_dict.items():
            print(f"Processing label: {label}, Number of images: {len(images)}")
            if not images:
                continue
            ...
            if not combined:
                print(f"No combined images for label: {label}")
                continue
...
def create_dataset(path, split_ratio, img_size, batch_size, folder_path_walk):
    triplet_generator = TripletGenerator(path, split_ratio, folder_path_walk)
